...
1. `WeakMap`是 ES6 中新增的一种集合类型，叫做“弱映射”。它和`Map`是兄弟关系，API 一致。

2. 特性

   1. 键名只能为对象
   2. 键名的引用为弱引用
      1. 强引用：垃圾回收机制不会主动释放内存，造成内存泄漏。
      2. 链接：[浅析 Map 和 WeakMap 区别以及使用场景 - 云+社区 - 腾讯云 (tencent.com)](https://cloud.tencent.com/developer/article/1862103)
...
1. WeakSet只能存储对象，而Set可以存储任意数据类型。
2. WeakSet存储的对象，如果没有其他的引用的话，这个对象将会被垃圾回收。这就是冠以 **Weak** 的原因，同时也意味着，WeakSet是不可枚举的，也就没有size。
...
web：在Vue3.0 Ts 启用 Element Plus

Web技术选型：

Vue3.0 + Ts + Element Plus + webpack + Echarts + sass + 设计模式 + three.js 

困难：

1. 之前多少有后端和数据库的基础，对其业务逻辑和编码过程知根知底，潜意识知道哪里可能出现Bug。但之前未接触硬件通讯，在编码时容易出现思维盲点。

2. 解决方法：总结后，编写Bug文档，逐步了解嵌入式前端的知识。
...
了解：至少看过一本入门书或者在大学学过相关课程，写过至少1个helloworld，并能动手修改。可以在实际需要时参考技术文档或帮助文件满足你的需要，基本知道此项技术在你运用是所起的作用，能够调用或者使用其根据规定提供给你的调用方式。


熟悉：写过数个或者数十个小程序，遇到问题需要上网搜思路和代码。熟悉编译工具的各项参数含义，知道一些小技巧。

熟练：能使用此技术实现软件需求并有经验的积累，在实现之前能做优化设计尽可能的实现模块或代码的重用。

精通：写过无数大型程序，看到别人的任何程序基本能猜测到实现原理。拿到需求可以轻松确定技术方案。能使用此技术优化性能或代码，做到最大可能的重用。
...
“了解”，即对需要理解的知识，但这种知识可能是局部的或点的，当涉及到知识时，要意识到自己知道;

“熟悉”，即对所需要的知识要有一个系统的了解，要找到知识之间的联系，要把知道在一起当作虚线，并能运用所学知识解决一些实际问题;

“熟练”，即对所需要具备的知识的理解全面，能够在每个知识点上将其串成线，在认识知识网络的基础上，并能够在实际工作中自由运用;

“精通”，即通过对所需要的知识的全面掌握，不仅对知识网络有清晰的认识，而且对相关领域的知识，能够同时灵活运用知识进行创新。                                                       
